# 🔥 ARF 模块开发参考文档：遥操作模块 (Teleoperation) V1.1
...
- **模式管理器 (Mode Manager):**运行中的DIL实例内部包含一个模式管理器。正常情况下，机器人处于AUTONOMOUS（自主）模式。当满足特定条件时（如DIL实例中的FailureDetectorComponent触发），DIL实例会切换到TELEOPERATION（遥操作）模式。
- **遥操作服务 (`teleop-service`):** 在`TELEOPERATION`模式下，此服务/组件被激活。它负责处理与操作员终端（VR/Web）的通信，并将操作员指令**直接或经过补偿算法处理后**，发送给`HAL`和`RTS`。
...
- **失败切换流程:** DIL实例中的FailureDetectorComponent检测到任务失败 -> 调用DIL实例中ModeManagerComponent的ActivateInterventionMode API -> 遥操作服务接管控制权，并开始记录专家数据。

- **在线残差学习流程:**
  
  1. DIL实例中的VLA策略组件输出一个**宏观动作** a_vla。
  
  2. 一个独立的ACR容器（**残差网络**）计算出**补偿动作** Δa。
  
  3. 最终指令 a_final = a_vla + Δa 被发送给HAL。
  
  4. 在遥操作示教时，Δa就是“人类专家的微操”与“VLA组件的原始想法”之间的差值，这些数据可用于训练残差网络ACR或直接微调VLA组件。
