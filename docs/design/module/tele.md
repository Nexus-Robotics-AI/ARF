...
作为ARF的“虚拟驾驶舱”和“专家示教器”，遥操作模块的核心使命是**建立一条从人类操作员到机器人本体的、高保真、低延迟、双向的沉浸式通信链路**。它不仅是机器人的“驾驶”工具，更是连接自主智能(`DIL`)与人类智慧的桥梁，是实现**失败学习**和**人机协同**闭环的关键。

主要应用场景包括：

- **✍️ 高保真示教数据采集:** 操作员通过VR等设备演示复杂任务，系统记录下高质量的、可用于模仿学习(IL)的完整数据流。
- **🤝 人机协同任务:** 人类操作员与`DIL`的自主策略协同工作，例如由人负责指定目标，由AI负责精细操作。
- **🚨 远程故障干预与修正:** 当`DIL`的自主任务失败时，无缝切换到遥操作模式，由人类专家进行修正。此修正过程本身就是宝贵的“失败数据”。
- **🔬 沉浸式远程调试:** 开发者通过双目视频和3D渲染，获得亲临现场(Telepresence)的体验，进行远程调试。
...
| **VR端**           | 双目视频流, `VirtualArmState` | 实时     | 视频延迟 < 150ms      |
