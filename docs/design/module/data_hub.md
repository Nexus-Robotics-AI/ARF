# 🔥 ARF 模块开发参考文档：数据中心 (Data Hub)  V1.1
...
- **📚 数据资产治理:** 对所有数据集进行版本化、元数据管理、**血缘追踪**和**质量监控**，确保数据的可追溯性、安全性和可靠性。
- **🔍 智能数据发现:** 为`训练模块`等消费端提供高效、灵活的数据查询接口，并支持基于自然语言的**语义搜索**，以快速发现最有价值的数据。
- **🌱 主动数据激活:** 与`训练模块`联动，通过**主动学习**等策略，智能地采样出能让模型训练效率最大化的“黄金数据”。
- **📊 硬件健康遥测数据存储:** 归档来自`HAL`层的海量、高频硬件遥测数据，为训练**预测性维护**模型提供数据基础。
- **🛡️ 用户个性化档案管理:** 安全、可靠地存储和管理来自`应用层`的**用户偏好与记忆**数据，并提供严格的隐私和权限控制。
- **📄 综合评测报告归档:** 存储和管理由`评测模块`生成的结构化评测报告和**Sim-to-Real差距分析**数据。

### 🏗️ 核心架构：湖仓一体 (Lakehouse Architecture) + 主动治理层 (Lakehouse + Active Governance Layer)
...
- **主动治理层 (Active Governance Layer):** 这是V1.1架构的核心扩展。它是一系列新的微服务，负责自动化地进行**数据血缘追踪**、**数据质量扫描**、**语义索引构建**和**生命周期管理**。

### ⚖️ 设计原则 (Design Principles)

- **📖 开放格式优先 (Open Formats First):** 所有数据都必须以开放的、社区标准的格式（Parquet, Arrow, Protobuf）存储，避免厂商锁定。
- **🔒 安全与合规:** 必须提供精细的访问控制（RBAC）和数据加密，确保数据安全，并支持GDPR等隐私合规要求。**在处理用户个性化档案等敏感数据时，必须提供数据脱敏、加密和严格的权限审计能力**。
- **🚀 存算分离 (Separation of Storage and Compute):** 存储层和计算层独立扩展，提供最大的灵活性和效率。
- **🔄 版本化一切 (Version Everything):** 所有的数据集和模型都必须是可版本化的，确保任何实验的可复现性。
- **🔍 透明可追溯 (Transparent & Traceable):** 任何数据资产的来源、加工历史和使用情况都应是清晰可查的。
...
| **DH7** | **自动化数据血缘与治理**    | **[V1.1+ 新增]** 系统必须能自动记录数据的完整处理链路，并提供接口查询任意数据集的血缘图。 | **高 (V1.1+)** |
| **DH8** | **数据质量监控与告警**      | **[V1.1+ 新增]** 必须包含自动化的数据质量扫描机制，能在数据出现异常（如格式、分布问题）时发出告警。 | **高 (V1.1+)** |
| **DH9** | **语义化数据搜索**          | **[V1.1+ 新增]** 必须提供超越键值查询的语义搜索能力，支持通过自然语言描述来发现相关数据。 | **中 (V1.1+)** |
...
### 🏛️ 3.4 主动治理服务 (Active Governance Service)



**数据管家角色**：这是一组新的后台微服务，负责数据的健康与秩序。

- **血缘追踪器 (Lineage Tracker):** 监听`Converter`和`Training`等模块的任务事件，自动构建和更新数据资产之间的血缘关系图谱。
- **质量扫描器 (Quality Scanner):** 定期对新入湖的数据进行剖析（Profiling）和扫描，执行预设的数据质量规则（如“图像分辨率不应低于720p”，“IMU数据不应有超过1秒的中断”），并将结果存入元数据层。



### 💡 3.5 智能发现服务 (Intelligent Discovery Service)



**数据导航员角色**：负责让海量数据变得“可以理解”和“易于发现”。

- **职责:** 消费`Data Hub`中的数据，利用多模态模型自动为数据（特别是图像和文本）提取丰富的语义标签。然后，将这些标签连同元数据一起构建到一个专门的搜索索引（如Elasticsearch）中，对外提供自然语言搜索和语义向量搜索API。
...
service DataHubService {
  ...
  // --- V1.1 新增 ---
  // [V1.1 新增] 执行一个语义搜索任务
  rpc SemanticSearch(SemanticSearchRequest) returns (ExecuteQueryResponse);
  // [V1.1 新增] 获取一个数据集的血缘信息
  rpc GetDataLineage(GetDataLineageRequest) returns (DataLineageResponse);
  // [V1.1 新增] 获取一个数据集的质量报告
  rpc GetDataQualityReport(GetDataQualityReportRequest) returns (DataQualityReport);
}

// 语义搜索请求
message SemanticSearchRequest {
    string natural_language_query = 1; // e.g., "a robot failing to grasp a red apple"
}
...
#### **第五阶段及以后：迈向主动数据引擎 (Towards Active Data Engine)**



*此部分为V1.1及后续版本规划，旨在激活数据价值。*

- **任务5.1: 实现数据血缘追踪**
  - **交付物:** `主动治理服务`能够监听`Converter`模块的任务事件，并成功生成一个简单的“输入数据集 -> 转换任务 -> 输出数据集”的血缘图。提供`GetDataLineage` API用于查询。
- **任务5.2: 开发数据质量监控原型**
  - **交付物:** 实现一个可配置的数据质量扫描器，能对新上传的数据执行至少3种类型的质量检查（如非空、范围、格式），并将不合格的数据标记出来，生成质量报告。
- **任务5.3: 实现语义搜索后端**
  - **交付物:** `智能发现服务`能够利用预训练的CLIP等多模态模型，为图像数据生成向量嵌入，并构建索引。`SemanticSearch` API能够接收文本输入，返回最相关的图像数据集。
- **任务5.4: 与主动学习联动**
  - **交付物:** 与`训练模块`团队协作，实现一个端到端的主动学习流程：`训练模块`识别出“困难”样本的特征，`Data Hub`能够根据这些特征，从海量数据中高效地采样出一个新的、高价值的数据集用于下一轮训练。
