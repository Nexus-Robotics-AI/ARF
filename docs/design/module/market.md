# 🔥 ARF 模块开发参考文档：插件市场 (Marketplace) V1.1
...
| **M6** | **资产类型支持**    | **[V1.1 更新]** 必须支持所有核心数字资产类型，包括HAL_DRIVER, ACR_ALGORITHM, **DIL_COMPONENT**, **DIL_INSTANCE_CONFIG**, TRAINING_SCRIPT, 和 APPLICATION_SUITE。 | **最高** |
...
enum PluginType {  
 ...
  TRAINING_SCRIPT = 3;  
  APPLICATION_SUITE = 4;  
  DIL_COMPONENT = 5; // 新增：可复用的决策组件  
  DIL_INSTANCE_CONFIG = 6; // 新增：一个完整的DIL实例配置  
}
