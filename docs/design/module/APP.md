# 🔥 ARF 模块开发参考文档：应用层 (Application) V1.1
...
- **应用套件 (Application Suite):** 这是一个**后端的、无界面的服务**。它的核心是一个**为特定场景组装好的DIL实例 (DIL Instance)**。这个DIL实例通过编排一组精选的DIL组件和ACR算法容器，来实现复杂的场景化业务逻辑。例如，“工业抓取套件”会加载一个包含高精度视觉搜索组件和精密操作策略的DIL实例。
- **用户界面 (User Interface):** 这是一个**独立的前端应用**（Web/移动端）。它通过`API`层提供的gRPC或RESTful接口与后端的“应用套件”进行通信，负责向用户展示信息和接收用户指令。
- **个性化与上下文引擎 (Personalization & Context Engine):** 这是V1.1架构的核心扩展。它是一个新的后端组件，负责管理用户模型，学习用户偏好，并融合上下文信息，为DIL实例的决策提供个性化依据。
...
- **技能加载器:** **[V1.1+]** 负责从插件市场下载已安装的“技能包”。一个技能包可以是：一个新的ACR原子能力。一个新的、可复用的DIL组件。一个完整的、可直接执行特定任务的DIL实例配置。
...
- **[V1.1 更新] 复杂逻辑调试:** 对于由多个DIL组件动态组合而成的复杂任务，开发者应使用Dev模块提供的**可视化调试套件**进行调试。特别是**行为树可视化器 (arf-cli dev debug bt)**，它是理解和排查DIL实例内部实时决策逻辑的首选工具。
