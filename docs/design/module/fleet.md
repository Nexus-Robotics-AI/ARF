...
作为ARF生态的“总指挥中心”，舰队管理模块的核心使命是**为部署在全球范围内的、数以万计的机器人（边缘节点）提供一个统一的、可视化的、自动化的管理与运维平台**。它将复杂的分布式系统管理工作，抽象为简单的API和UI操作。
...
### 🏗️ 核心架构：基于云原生边缘计算 (Cloud-Native Edge Computing)

我们采用业界主流的云原生边缘计算架构，将Kubernetes的能力从云端延伸到边缘端。

- **云端控制器 (Cloud Controller):** 运行在云端`ARF Cloud Plane`的核心服务。它作为管理平面的大脑，负责接收用户的管理指令，并维护所有边缘节点的期望状态（Desired State）。
...
- **指令下发器:** 当检测到期望状态与报告的实际状态不一致时，生成相应的指令（如`DeployPlugin`）并安全地发送给边缘代理。
