...
| **V4** | **可视化调试工具**                       | **[V1.1+ 新增]** 必须提供至少三种Web可视化调试工具（如Topic查看、行为树可视化、坐标系可视化）。 | **最高 (V1.1+)** |
| **V5** | **一键式云开发环境**                     | **[V1.1+ 新增]** `arf-cli`必须支持命令，用于在云端自动创建和连接到一个预配置的、基于容器的开发环境。 | **高 (V1.1+)**   |
| **V6** | **时间旅行调试**                         | **[V1.1+ 新增]** `arf-cli`必须支持加载一个`DMS`记录文件，并将其内容回放到一个本地或云开发环境中，以复现问题。 | **高 (V1.1+)**   |
