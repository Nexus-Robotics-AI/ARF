# 🔥 ARF 模块开发参考文档：数据转换模块 (Converter)  V1.1
...
- **🤝 人机协同标注:** 与数据标注平台无缝集成，实现“机器自动预标注、人类专家修正”的高效协同标注流程。
- **🤖 仿真数据增强:** 以真实数据为蓝本，调用`仿真模块`大规模生成带有完美标注的多样化合成数据，以解决长尾问题。
- **🌊 数据漂移检测:** 自动化地分析新数据与存量数据的统计分布差异，提前预警模型性能下降的风险。

### 🏗️ 核心架构：“流水线即代码”+ “智能算子库” (Pipeline-as-Code + Intelligent Operator Library)
...
- **智能算子库 (Intelligent Operator Library):** V1.1架构的核心扩展是提供一系列更高阶的“智能算子”，如**人机协同标注算子**、**仿真增强算子**和**数据分析算子**。
...
- **🤝 协同智能 (Collaborative Intelligence):** 设计必须支持人与机器在数据处理环节（尤其是标注）的无缝协作。
...
| **C6** | **人机协同标注**   | **[V1.1+ 新增]** 必须提供标准算子，能将低置信度的自动标注结果发送到标注平台，并在人工标注完成后，将结果拉取回流水线。 | **高 (V1.1+)** |
| **C7** | **仿真数据增强**   | **[V1.1+ 新增]** 必须提供标准算子，能调用`仿真模块`API，根据输入数据生成多样化的合成数据。 | **高 (V1.1+)** |
| **C8** | **数据漂移检测**   | **[V1.1+ 新增]** 必须提供标准化的流水线模板，用于自动化分析和对比数据集之间的统计分布差异。 | **中 (V1.1+)** |
...
- **人机协同标注算子 (Human-in-the-Loop Operator):**
  - **职责:** 这是一个特殊的算子，它接收自动标注模型的结果。如果结果置信度低于配置的阈值，它会调用标注平台的API创建任务。此算子会暂停流水线的执行，并定期轮询标注任务的状态。一旦任务完成，它会下载标注结果，并将其作为输出传递给流水线的下一环。
- **仿真增强算子 (Simulation Augmentation Operator):**
  - **职责:** 该算子接收一个或多个真实世界的“资产”（如一个物体的3D模型，一段轨迹）。它会调用`仿真模块`的API，在一个或多个并行仿真实例中，将这些资产置于不同的环境（不同的光照、背景、遮挡）下，生成大量的、带有完美标注的合成数据，并将其作为算子的输出。
- **数据分析与漂移检测算子 (Data Drift & Analytics Operator):**
  - **职责:** 这类算子专门用于数据分析。它会加载一个或多个数据集，计算其关键的统计指标（如类别分布、尺寸分布等），并能对比两个数据集的分布差异（如使用KL散度等方法），最终生成一份可视化的分析报告或一个JSON格式的漂移分数。
...
message RunConversionRequest {
    ...
    // [V1.1 新增-可选] 关联的外部服务，用于UI展示和权限打通
    optional string labeling_project_id = 4;    
...
#### **第五阶段及以后：迈向智能数据策展 (Towards Intelligent Data Curation)**



*此部分为V1.1及后续版本规划，旨在提升数据处理的自动化和智能化水平。*

- **任务5.1: 实现人机协同标注算子**
  - **交付物:** 开发一个能与开源标注平台Label Studio集成的标准算子。在一个测试流水线中，验证该算子能够成功地将数据推送至Label Studio，并在人工标注完成后将结果拉取回来。
- **任务5.2: 开发仿真增强算子**
  - **交付物:** 开发一个能调用`仿真模块`API的标准算子。在一个测试流水线中，输入一个3D物体模型，该算子能成功生成100张该物体在不同随机背景和光照下的渲染图像。
- **任务5.3: 建立数据漂移检测流水线**
  - **交付物:** 提供一个标准化的流水线模板和一组数据分析算子。在一个测试中，输入两个具有明显分布差异的数据集，该流水线能成功地量化其差异并生成一份可视化报告。
- **任务5.4: 完善官方算子库**
  - **交付物:** 将上述高级功能的核心算子进行加固和优化，并编写详细的文档和示例，将其作为官方标准算子库的一部分发布，供所有开发者使用。
